...
export default function Dashboard({ profile, onSelectGame }) {
  ...
  const metricCards = [
    { label: t('nav.games'),             value: stats.games,   icon: CalendarDays, color: 'sky' },
    { label: t('nav.players'),           value: stats.players, icon: Users,        color: 'violet' },
    { label: t('dash.metric_events'),    value: stats.events,  icon: Zap,          color: 'amber' },
    { label: t('nav.teams'),             value: stats.teams,   icon: Shield,       color: 'emerald' },
  ]
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto w-full">
      ...
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--tx4)' }}>{t('dash.subtitle')}</p>
      </div>
          ...
          return (
            <div key={label} className="rounded-2xl p-4 md:p-5" style={cardStyle}>
              ...
              <div className="text-2xl md:text-3xl font-black"
                ...
                {loading ? '—' : value.toLocaleString(locale)}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        ...
        <div className="rounded-2xl overflow-hidden" style={cardStyle}>
          <div className="flex items-center justify-between px-5 py-4" style={innerBorder}>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold" style={{ color: 'var(--tx)' }}>{t('dash.live_title')}</h3>
            </div>
            <span className="text-xs" style={{ color: 'var(--tx4)' }}>{new Date().toLocaleDateString(locale)}</span>
          </div>

          {liveGames.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center px-4">
              <Clock size={28} className="mb-3" style={{ color: 'var(--tx5)' }} />
              <p className="text-sm" style={{ color: 'var(--tx4)' }}>{t('dash.no_live')}</p>
            </div>
          ) : (
            <div>
              {liveGames.map((g) => (
                <div key={g.id} className="flex items-center justify-between px-5 py-3.5 gt-list-row"
                  ...
                  <div>
                    ...
                    <div className="text-xs mt-0.5" style={{ color: 'var(--tx4)' }}>
                      {new Date(g.game_date).toLocaleDateString(locale)}
                    </div>
                  </div>
                  <button
        ...
        <div className="rounded-2xl overflow-hidden" style={cardStyle}>
          <div className="flex items-center justify-between px-5 py-4" style={innerBorder}>
            <h3 className="text-sm font-bold" style={{ color: 'var(--tx)' }}>{t('dash.recent_title')}</h3>
          ...
          {recentGames.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center px-4">
              ...
              <p className="text-sm" style={{ color: 'var(--tx4)' }}>{t('dash.no_games')}</p>
            </div>
          ) : (
            <div>
              {recentGames.map((g) => (
                <div key={g.id} className="flex items-center justify-between px-5 py-3.5 gt-list-row"
                  ...
                  <span className="text-[0.65rem] font-bold px-2 py-0.5 rounded-full flex-shrink-0"
                    ...
                    {g.status === 'finished' ? t('status.finished') : t('status.active')}
                  </span>
                </div>
              ))}
            </div>
